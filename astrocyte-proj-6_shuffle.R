...
for(curPeak in loopPeaksV) {
    ...
    curLmFit = phylolm(Homeotherm ~ curPeak, data=speciesDetailedInfoTmpF, phy=pruned_tree, model="BM")
...
options(warn=0)
file_name <- sprintf("Peak_Phylolm/peakPhyloResults_astrocyte_mouse_new.csv")
...
options(warn=0)
file_name <- sprintf("Peak_Phylolm/peakPhyloResults_vip_mouse_new.csv")
write.csv(peakPhyloResultsF, file = file_name, row.names = FALSE)
flush.console()
Sys.sleep(1)
